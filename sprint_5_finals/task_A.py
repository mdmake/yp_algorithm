...
-- ВРЕМЕННАЯ СЛОЖНОСТЬ --
Вставка n  элементов в кучу -- O(log1)+O(log2)+...+O(logn). Оценивая сверху, получаем O(n log n)
Оценим стоимость извлечения элемента из кучи споследующим восстановлением стуктуры кучи:
Нам необходимо раз извлечь элемент с последующим восстановлением структуры
Так как после каждого извлечения элемента размер кучи становится меньше,
то стоимость восттановления можно записать как O(log n)+O(log n-1)+...+O(log2) + O(log1).
Оценивая сверху, получаем O(n log n)
