...
-- ВРЕМЕННАЯ СЛОЖНОСТЬ --
Поиск элемента в дереве - O(h), где h-высота дерева. В худшем случае - O(n)
Поиск замещающего элемента --- O(h_1), где h_1-высота левого поддерева. В худшем случае - O(n)
Итого, временная сложность O(h), в худшем случае - O(n),
